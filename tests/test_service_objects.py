...
from legsy.service_objects import color_service_object, size_service_object
...
async def test_service_object_get_by_name(
    size, color, model, service_object, name, async_session_maker: sessionmaker
):
...
async def test_size_service_object_delete(
        size, async_session_maker: sessionmaker
):
    session: AsyncSession = async_session_maker()
...
async def test_size_service_object_rename(
        size, async_session_maker: sessionmaker
):
    session: AsyncSession = async_session_maker()
    await size_service_object.update(
        size,
        SizeUpdate(name='Новое название размера'), session
    )
    await session.commit()
    count = await session.scalar(
        select(func.count()).select_from(Size)
...
async def test_size_service_object_get_by_id(
        size, async_session_maker: sessionmaker
):
    session: AsyncSession = async_session_maker()
